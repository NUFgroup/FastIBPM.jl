...
## 2. Grid and Body Definition

We define a multilevel grid with resolution `h = 0.02` and a circular cylinder body.
...
Define time step, Reynolds number, and freestream inflow:
...
We define the solver loop that:
...
```julia
soln_path = joinpath(OUTDIR, "$(CASE).h5")
