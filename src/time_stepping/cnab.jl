...
# ===========================================================================
# Manifold-projection (IMAP) time step
# ===========================================================================
#
# One CNAB step of the fractional-step method with no-slip enforced by projection
# instead of by a boundary force:
#
#   (1) prediction  A q*    = r1            q* = Bᴺ r1        `prediction_step!`
#   (2) pressure    GᵀBᴺG φ = Gᵀ q* - r2                      `pressure_step!`
#   (3) correction  q^{n+1} = q* - Bᴺ G φ                     `correction_step!`
#   (4) force       f̃ = (RᵀR)⁻¹Rᵀ[...] - (RᵀR)⁻¹Rᵀ[G φ]       `recover_force!`
#                       (accumulated across steps 1, 3 and 4 — each projection
#                        already forms the multiplier it needs)
#
# with A = (1/Δt)I - (1/2Re) P L and Bᴺ the projected viscous inverse
# (`Ainv_IMAP!`). Compared with the `IBPM` pipeline above, the multiplier has lost
# its force block (`λ = φ`), so step (2) is grid-sized and step (3) spreads no
# force — the body enters only through `P`. Step (4) is diagnostic: `f̃` is an
# output, not an unknown, so it never feeds back into the solve.
#
# NAMING: step (3) is what `IBPM`/`FastIBPM` call `projection_step!` — the
# fractional-step pressure projection. It is named `correction_step!` here, as in
# the IMAP document, because in this formulation "projection" already means the
# *constraint* projection `P`, and the two must not be confused: `P` enforces
# no-slip and lives inside steps (1) and (2); this one enforces incompressibility.
#
