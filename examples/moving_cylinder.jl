...
# %%
h = 0.03  # grid cell size
gridlims = SA[-15.0 15.0; -15.0 15.0]
grid = Grid(;
...
function solution(file; tf, snapshot_freq)
    ...
    body_pts = create_dataset(body_group, "points", T, (2, length(body.x_ref)))
    body_pts[:, :] = reduce(hcat, body.x_ref)
    ...
    # per-snapshot body positions for animation of moving body
    body_snap = create_dataset(
        snapshot_group, "body_points", T, (2, length(body.x_ref), n_snapshot)
    )
    ...
    for lev in 1:grid.levels
        X, Y = coord(grid, Loc_ω(3), (i0:(i0 + Nx_ω - 1), j0:(j0 + Ny_ω - 1)), lev)
        x_ds[:, lev] = X[:, 1]
        y_ds[:, lev] = Y[:, 1]
    end
    ...
    @showprogress desc = "solving" for _ in 0:round(Int, tf / dt)
        ...
        if sol.i in i_snapshot
            ...
            body_snap[:, :, i] = reduce(hcat, sol.points.x)
        end
    end
end
...
if isfile(soln_path)
    ...
    h5open(soln_path, "cw") do file
        #solution(file; tf=1, snapshot_freq=10)
        solution(file; tf=4.0, snapshot_freq=20)
    end
end
...
# Comparison with Taira & Colonius (2007)
ref_data = readdlm(joinpath(OUTDIR, "dataset_colonius2007.csv"), ',')
scatter!(p, ref_data[:, 1], ref_data[:, 2];
         color=:black, ms=3, markershape=:circle, label="Cd (Colonius 2007)")
