...
h = 0.075
gridlims = SA[-1 6; -2.0 2.0]
grid = Grid(;
    h, n=@.(round(Int, (gridlims[:, 2] - gridlims[:, 1]) / h)), x0=gridlims[:, 1], levels=2
)

# %%
r = 0.5  # cylinder radius
S = 2π * r  # cylinder circumference
n_ib = round(Int, S / (2 * h)-1)  # number of immersed boundary points
...
h5open(soln_path, "r") do file
    ...
    gif(anim, joinpath(OUTDIR, "$(CASE)_vorticity.gif"); fps=50)
end
...
h5open(soln_path, "r") do file
    ...
    @showprogress for i in eachindex(t)
        ...
        for lev in levels
            ...
            # mask non-finite values so contourf does not choke on NaN/Inf
            finite_mask = isfinite.(zplot)

            # clamp only finite values
            zplot[finite_mask] .= clamp.(zplot[finite_mask], -ωlim, ωlim)

            # leave non-finite values as NaN
        ...
        title!(p, @sprintf("t = %.2f", t[i]))
        frame(anim, p)
    end

    gif(anim, joinpath(OUTDIR, "$(CASE)_vorticity_contour.gif"); fps = 24)
end
